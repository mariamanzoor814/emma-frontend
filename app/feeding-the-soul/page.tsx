...
const SLUG = "feeding-the-soul";

type CmsBlock = { key: string; value: any };

function getBlock(blocks: CmsBlock[], key: string, field = "text"): string {
  return blocks.find(x => x.key === key)?.value?.[field] ?? "";
}

function getList(blocks: CmsBlock[], key: string): any[] {
  return Array.isArray(blocks.find(x => x.key === key)?.value?.items)
    ? blocks.find(x => x.key === key)!.value.items
    : [];
}

export default async function FeedingSoulPage({
  searchParams,
}: {
  ...
  const heroTitle = getBlock(page.blocks, "hero.title") || "Feeding the Soul";
