...
export const HeroCarousel: React.FC<HeroCarouselProps> = ({
  ...
  return (
    <section className="mx-auto mt-4 max-w-7xl px-4">
      <div
        className="relative flex flex-col justify-between rounded-3xl px-8 py-8 md:flex-row md:items-center md:gap-10"
        ...
        {slides.length > 1 && (
          <>
            <button
              type="button"
              onClick={goPrev}
              className="absolute left-4 top-1/2 hidden h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-sm shadow-sm hover:bg-white md:flex"
            >
              ‹
            </button>
            <button
              type="button"
              onClick={goNext}
              className="absolute right-4 top-1/2 hidden h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-sm shadow-sm hover:bg-white md:flex"
            >
              ›
            </button>
          </>
